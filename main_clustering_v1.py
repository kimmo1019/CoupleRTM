...
class RoundtripModel(object):
    def __init__(self, g_net, h_net, dx_net, dx_cat_net, dy_net, q_net, x_sampler, y_sampler, nb_classes, data, pool, batch_size, alpha, beta, is_train):
        self.data = data
        self.g_net = g_net
        self.h_net = h_net
        self.dx_net = dx_net
        ...
        #gradient penalty for x_cat
        epsilon_x_cat = tf.random_uniform([], 0.0, 1.0)
        x_cat_hat = epsilon_x_cat * self.x_onehot + (1 - epsilon_x_cat) * self.x_onehot_
        dx_cat_hat = self.dx_cat_net(x_cat_hat)
        grad_x_cat = tf.gradients(dx_cat_hat, x_cat_hat)[0] #(bs,x_dim)
        grad_norm_x_cat = tf.sqrt(tf.reduce_sum(tf.square(grad_x_cat), axis=1))#(bs,)
        self.gpx_cat_loss = tf.reduce_mean(tf.square(grad_norm_x_cat - 1.0)*10)
        ...
        self.dx_cat_optim = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
                .minimize(self.dx_cat_loss+self.gpx_cat_loss, var_list=self.dx_cat_net.vars)
    ...
    def train(self, nb_batches, patience):
        ...
        #weights=np.array([666, 77, 75, 373, 92, 94])/1377.
        #weights = np.array([44, 375, 281])/700.
...
################################### iteratively updating ############################
            ...
            if (batch_idx+1)%50 == 0:
                ...
                print weights
                print self.estimate_weights(use_kmeans=True)
...
if __name__ == '__main__':
    ...
    dx_cat_net = model.Discriminator(input_dim=nb_classes,name='dx_cat_net',nb_layers=2,nb_units=128)
    ...
    xs = util.Mixture_sampler(nb_classes=nb_classes,N=10000,dim=x_dim,sd=1,scale=1)
    ...
    RTM = RoundtripModel(g_net, h_net, dx_net, dx_cat_net, dy_net, q_net, xs, ys, nb_classes, data, pool, batch_size, alpha, beta, is_train)
